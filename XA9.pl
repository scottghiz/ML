...
$sql = "\"   SELECT sessionid,starttimestamp,endtimestamp,accountnumber,region,division,authenticationtype,channel,fiscalmonthenddate,fiscalweekenddate,fullpenultimate,fullthemeid,fullpath,initialcontentcode,initialintentcode,virtualhold,sessiontype,truechatflag,chatflag,socialflag,callflag,chatflag7day,socialflag7day,callflag7day,chatflagsamequeue,socialflagsamequeue,callflagsamequeue,chatflag7daysamequeue,socialflag7daysamequeue,callflag7daysamequeue,contactflag,contactflag7day,uniquequeries,lob,callgroup,interactiongroup,interactiontype,didselfservice,didinfodelivery,didmessaging,didnavigation,endstatus,hasvideo,hashsd,hascdv,hasxh,platform,createtimestampepoch,id,statusdate,statustype,source_name,audit_dml_action_cd,audit_job_id,audit_inserted_ts,audit_updated_ts,current_record_ind,eff_dt,eff_ts,exp_ts,versionid,contentversion,unique15minsession,unique15mincontactflag,unique15mincontactflag7day,day_id FROM meld.jrnl.v_virtualassistantsummary 
  WHERE initialintentcode NOT LIKE '%null%' 
  AND initialintentcode NOT LIKE '%medallia%' 
  AND initialintentcode NOT LIKE '%onetimepay%' 
  AND initialintentcode NOT LIKE '%eligibility%' 
  AND initialintentcode NOT LIKE '%rental%' 
  AND initialintentcode NOT LIKE '%realtimecredit%' 
  AND initialintentcode NOT LIKE '%billing%' 
  AND initialintentcode NOT LIKE '%scheduled%' 
  AND initialintentcode NOT LIKE '%thankyou%' 
  AND initialintentcode NOT LIKE '%onetimepayment%' 
  AND initialintentcode NOT LIKE '%reminder%' 
...
while (1) {
  ...
  system("cat $jpath/$loop_count.json $jpath/all9.json > $jpath/temp9.json");
  system("mv $jpath/temp9.json $jpath/all9.json");
...
open(FINAL,">","$jpath/final9.json") || die("cannot open $jpath/final9.json\n");
open(ALL,"$jpath/all9.json") || die("cannot open $jpath/all9.json\n");
while($row=<ALL>){
  chomp($row);
  if($row =~ m/\"data\"/){
    print FINAL "$row\n";
...
open(NEW,">","$csvpath/final9.csv") || die("cannot open $csvpath/final9.csv\n");
open(DATA,"$jpath/final9.json") || die("cannot open $jpath/final9.json\n");

...
while (1) {
  ...
  system("cat $jpath/$loop_count.json $jpath/all5.json > $jpath/temp5.json");
  system("mv $jpath/temp5.json $jpath/all5.json");
...
open(FINAL,">","$jpath/final5.json") || die("cannot open $jpath/final5.json\n");
open(ALL,"$jpath/all5.json") || die("cannot open $jpath/all5.json\n");
while($row=<ALL>){
  chomp($row);
  if($row =~ m/\"data\"/){
    print FINAL "$row\n";
...
open(NEW,">","$csvpath/final5.csv") || die("cannot open $csvpath/final5.csv\n");
open(DATA,"$jpath/final5.json") || die("cannot open $jpath/final5.json\n");

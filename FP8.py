...
xa_tgts_file = open("DATA/XA_targets.txt","r")
xa_tgts_list = xa_tgts_file.readlines()

new_list = []
for x in range(len(xa_tgts_list)):
    z = xa_tgts_list[x].replace("\n","")
    new_list.append(z)
    #print(z)

inname = "CSV/final8.csv"
df = pd.read_csv(inname, compression=None, header=0, sep=',',low_memory=False)
df_7 = df.filter(['fullpath'],axis=1)
fp_list = df_7.values.tolist()
new_list_count = 0
fp_list_count = 0
tgt_list = []
for y in range(len(fp_list)):
  fp_list_count = fp_list_count + 1
  new_list_count = 0
  for x in range(len(new_list)):
    new_list_count = new_list_count + 1
    if str(new_list[x]) in str(fp_list[y]):
      #print("match: " + str(new_list_count) + " : " + str(fp_list_count) + " --- " + str(new_list[x]) + " --- " + str(fp_list[y]))
      tgt_list.append(str(new_list[x]))

tgt_uniq = []
for i in tgt_list:
    if i not in tgt_uniq:
        tgt_uniq.append(i)
...
print()
print("=== tgt_list length ===")
print(len(tgt_list))
print()
print("=== tgt_uniq length ===")
print(len(tgt_uniq))
